...
def build_prompt(payload: dict) -> str:
    ...
    prompt = f"""أنت محلل فني محترف بالأسواق المالية. حللي البيانات التالية القادمة من TradingView
وأعطني تحليل مرتب بالعربي يتضمن:

1. تحليل فني مختصر (اتجاه السعر، القوة/الضعف حسب المؤشرات المتوفرة)
2. أقرب مستويات دعم ومقاومة متوقعة بناءً على السعر الحالي والـ high/low
3. إشارة تداول واضحة: (شراء / بيع / انتظار) مع درجة ثقة تقريبية ومستوى وقف الخسارة المقترح
4. ملخص عام سريع عن حالة السهم/العملة في هذا التايم فريم
...
مهم: هذا تحليل آلي مبني على بيانات محدودة وليس نصيحة استثمارية، وضّح هذا بجملة قصيرة في النهاية فقط."""
    return prompt
...
def webhook():
    ...
    send_telegram(f"📊 تحليل {payload.get('ticker', '')}:\n\n{analysis}")
